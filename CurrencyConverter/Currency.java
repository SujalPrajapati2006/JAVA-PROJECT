...
public class Currency {
    ...
    // Simple main method to show example output
    public static void main(String[] args) {
        ArrayList<Currency> currencies = Currency.init();

        // Example: Convert 100 USD to EUR
        Currency usd = currencies.get(0); // US Dollar
        Double rateToEUR = usd.getExchangeValues().get("EUR");
        Double convertedAmount = Currency.convert(100.0, rateToEUR);

        System.out.println("100 USD to EUR: " + convertedAmount);

        // Print exchange rates of GBP
        Currency gbp = currencies.get(2); // British Pound
        System.out.println("\nExchange rates for GBP:");
        for (String key : gbp.getExchangeValues().keySet()) {
            System.out.println("GBP to " + key + " = " + gbp.getExchangeValues().get(key));
        }
    }
}
